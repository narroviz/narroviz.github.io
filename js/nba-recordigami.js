...
var BG_DARK_GRAY = "#5F5F5F";
var BG_GRAY = "#828282";
var BG_LIGHT_GRAY = "#d8d8d8";
var BG_VERY_LIGHT_GRAY = "#ececec";
var PATH_ID = '#paths-wrapper';
var NBA = 'NBA';
var WNBA = 'WNBA';
var LEAGUE = NBA;
var DEFAULT_FILTER_NAME = '--- All Teams ---';
...
function drawBaseTiles(league) {
  ...
  dimensions['legendWidth'] = (dimensions.width - dimensions.margin.left - dimensions.margin.right) * 1; // on mobile
  ...
  var tiles = tilesGroup.selectAll(".rect-tile").data(defaultTileData, function (d) {
    ...
    return "".concat(extra, "tile-").concat(winAccessor(d), "-").concat(lossAccessor(d));
  ...
  }).style("fill", BG_VERY_LIGHT_GRAY).style("opacity", 1).attr("rx", 0).attr("ry", 0);
  var winLossGroup = bounds.append("g");
  var winsText = winLossGroup.append("text").text("Wins").attr("x", -12).attr("y", -10).attr("font-size", FONT_SIZE).attr("text-anchor", "middle").attr("fill", BG_GRAY).attr("id", "".concat(extra, "wins-text")); // .attr("opacity", 0.5)

  var lossesText = winLossGroup.append("text").text("Losses").attr("id", "".concat(extra, "losses-text")).attr("x", dimensions.boundedWidth + 10).attr("y", dimensions.boundedHeight + 17).attr("font-size", FONT_SIZE).attr("text-anchor", "start").attr("alignment-baseline", "middle").attr("fill", BG_GRAY); // on desktop

  if (_isMobile.default.any()) {
    d3.select("#".concat(extra, "losses-text")).remove();
    lossesText = winLossGroup.append("text").text("Losses").attr("x", dimensions.boundedWidth / 2).attr("y", dimensions.boundedHeight + 35).attr("font-size", FONT_SIZE).attr("text-anchor", "middle").attr("alignment-baseline", "middle").attr("fill", BG_GRAY);
  }
  ...
  }).attr("font-size", FONT_SIZE).attr("text-anchor", "middle").attr("alignment-baseline", "middle").attr("fill", BG_GRAY);
  ...
  }).attr("y", dimensions.boundedHeight + 17).attr("font-size", FONT_SIZE).attr("text-anchor", "middle").attr("alignment-baseline", "middle").attr("fill", BG_GRAY);
  var zeroLabel = bounds.append("text").text("0").attr("x", -12).attr("y", dimensions.boundedHeight + 17).attr("font-size", FONT_SIZE).attr("text-anchor", "start").attr("alignment-baseline", "middle").attr("fill", BG_GRAY);
...
function highlightGroup(group, animationTime, color) {
  group.transition("highlight-group").duration(animationTime).style("fill", color).style("stroke", color).style("stroke-opacity", 1);
}

function fadeGroup(group, animationTime, color) {
  group.transition("fade-group").duration(animationTime).style("fill", color).style("stroke", color).style("stroke-opacity", 0);
}

function highlightClass(classId, animationTime, color) {
  var classIdString = ".".concat(classId);
  d3.selectAll(classIdString).transition("highlight-class-".concat(classIdString)).duration(animationTime).style("fill", color).style("stroke", color).style("stroke-opacity", 1);
}

function fadeClass(classId, animationTime, color) {
  var strokeOpacity = arguments.length > 3 && arguments[3] !== undefined ? arguments[3] : 0;
  var classIdString = ".".concat(classId);
  d3.selectAll(classIdString).transition("fade-class-".concat(classIdString)).duration(animationTime).style("fill", color).style("stroke", color).style("stroke-opacity", strokeOpacity);
}
...
function drawVoronoi(years, recordigamiData, xScale, yScale, dimensions, bounds, onMouseEnter, onMouseLeave, onMouseMove) {
  var shouldJitter = arguments.length > 9 && arguments[9] !== undefined ? arguments[9] : false;
  bounds.selectAll(".voronoi").remove();

  var _getVoronoiPointsAndM = getVoronoiPointsAndMetadata(years, recordigamiData, xScale, yScale, shouldJitter),
      _getVoronoiPointsAndM2 = _slicedToArray(_getVoronoiPointsAndM, 2),
      voronoiPoints = _getVoronoiPointsAndM2[0],
      voronoiMetadata = _getVoronoiPointsAndM2[1];
...
function randomDecimalFromInterval(min, max) {
  var numDecimalPoints = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : 5;
  // min and max included 
  return parseFloat((Math.random() * (max - min) + min).toFixed(numDecimalPoints));
}

function getVoronoiPointsAndMetadata(seasons, recordigamiData, xScale, yScale) {
  var shouldJitter = arguments.length > 4 && arguments[4] !== undefined ? arguments[4] : false;
  var gamesSeen = {};
  var points = [];
  var pointsMetadata = [];
  ...
  for (var i = 0; i < seasons.length; i++) {
    ...
    if (year in recordigamiData) {
      ...
      (function () {
        var recordigamiGames = recordigamiData[year];
        var recordigamiKeys = Object.keys(recordigamiGames);
        var gamesWins = recordigamiKeys.map(function (game) {
          return recordigamiGames[game].win;
        });
        var gamesLosses = recordigamiKeys.map(function (game) {
          return recordigamiGames[game].loss;
        });
        var gamesXValues = recordigamiKeys.map(function (game) {
          return xScale(recordigamiGames[game].loss);
        });
        var gamesYValues = recordigamiKeys.map(function (game) {
          return yScale(recordigamiGames[game].win);
        });
        var gamesTeams = recordigamiKeys.map(function (game) {
          return recordigamiGames[game].team;
        });
        var gamesYear = recordigamiKeys.map(function (game) {
          return recordigamiGames[game].year;
        });
        var gamesPrimaryColor = recordigamiKeys.map(function (game) {
          return recordigamiGames[game].primary_color;
        });

        for (j = 0; j < gamesXValues.length; j++) {
          var gameWin = gamesWins[j];
          var gameLoss = gamesLosses[j];
          var gameX = shouldJitter ? (gamesXValues[j] + randomDecimalFromInterval(.00000001, .00000002, 11)).toPrecision(11) : gamesXValues[j].toPrecision(11);
          var gameY = shouldJitter ? (gamesYValues[j] + randomDecimalFromInterval(.00000001, .00000002, 11)).toPrecision(11) : gamesYValues[j].toPrecision(11);
          var gameTeam = gamesTeams[j];
          var gameYear = gamesYear[j];
          var gamePrimaryColor = gamesPrimaryColor[j];
          var gameKey = "".concat(gameWin, "_").concat(gameLoss);
...
function getNonRoundedIntervalArray(start, end, intervalLength) {
  var startInterval = start - start;
  var endInterval = end - start;
  var numIntervals = Math.floor((endInterval - startInterval) / intervalLength);
  var intervals = [start];

  for (var i = 0; i < numIntervals; i++) {
    var currentInterval = intervals[i] - start + intervalLength;
...
function drawRecordigami(_x2) {
  return _drawRecordigami.apply(this, arguments);
}

function _drawRecordigami() {
  _drawRecordigami = _asyncToGenerator(
  /*#__PURE__*/
  regeneratorRuntime.mark(function _callee2(league) {
    var _drawBaseTiles, _drawBaseTiles2, wrapper, bounds, dimensions, tiles, tilesGroup, yearIntervals, xScale, yScale, recordigamiData, seasonData, teamData, teams, suggestionGroup, onSuggestionClick, onExitClick;

    return regeneratorRuntime.wrap(function _callee2$(_context2) {
      while (1) {
        switch (_context2.prev = _context2.next) {
          case 0:
            onExitClick = function _ref2(e) {
            ...
            onSuggestionClick = function _ref(e, suggestionIndex) {
              ...
              drawRecordigamiByTeam(league, DEFAULT_TEAM, recordigamiData, seasonData, teamData, wrapper, bounds, dimensions, tiles, tilesGroup, yearIntervals, xScale, yScale, suggestionDecade); // d3.select(".pac-container").style("height", 0)
            };

            _drawBaseTiles = drawBaseTiles(league, ".recordigami-container", "recordigami-", false), _drawBaseTiles2 = _slicedToArray(_drawBaseTiles, 8), wrapper = _drawBaseTiles2[0], bounds = _drawBaseTiles2[1], dimensions = _drawBaseTiles2[2], tiles = _drawBaseTiles2[3], tilesGroup = _drawBaseTiles2[4], yearIntervals = _drawBaseTiles2[5], xScale = _drawBaseTiles2[6], yScale = _drawBaseTiles2[7];
            _context2.next = 5;
            return d3.json("./../assets/data/".concat(league, "_recordigami.json"));

          case 5:
            recordigamiData = _context2.sent;
            _context2.next = 8;
          ...
          case 11:
            ...
            if (DEFAULT_TEAM === "") {
              ...
              d3.select("#basketball-team-input").style("color", BG_GRAY);
            }
            ...
            d3.select(".typeahead").style("border", "1px solid ".concat(BG_GRAY)).style("border-radius", '30px');
...
function drawRecordigamiByTeam(_x3, _x4, _x5, _x6, _x7, _x8, _x9, _x10, _x11, _x12, _x13, _x14, _x15, _x16) {
  return _drawRecordigamiByTeam.apply(this, arguments);
}

function _drawRecordigamiByTeam() {
    ...
    return regeneratorRuntime.wrap(function _callee3$(_context3) {
      while (1) {
        switch (_context3.prev = _context3.next) {
          case 0:
            onRecordigamiMouseMove = function _ref6(datum) {
              var wins = datum['win'];
              var losses = datum['loss'];
              var team = datum['team'];
              var year = datum['year'];
              ...
              var primaryColor = datum['primary_color'];
              ...
              if (mouseTotal > 1.15 * NUM_GAMES) {
                ...
                hoverLoss.text(losses).style("opacity", 0); // hoverYear.text(year).style("opacity", 0)

                if (![DEFAULT_FILTER_NAME, ''].includes(filterTeam)) {
                  hoverTeam.text("".concat(year)).style("opacity", 0).style("fill", "black");
                  hoverSquare.style("stroke", "black");
                } else {
                  hoverTeam.text("".concat(team, " (").concat(year, ")")).style("opacity", 0).style("fill", primaryColor);
                ...
                if (![DEFAULT_FILTER_NAME, ''].includes(filterTeam)) {
                  hoverTeam.text("".concat(year)).style("opacity", 1).style("fill", "black");
                } else {
                  hoverTeam.text("".concat(team, " (").concat(year, ")")).style("opacity", 1).style("fill", primaryColor);
                }
            ...
            onRecordigamiMouseLeave = function _ref5(datum) {
              hoverSquare.style("opacity", 0);
              hoverWin.style("opacity", 0);
              hoverHyphen.style("opacity", 0);
              hoverLoss.style("opacity", 0); // hoverYear.style("opacity", 0)

              hoverTeam.style("opacity", 0);
              d3.selectAll("*[id^=num-recordigami]").style("opacity", 1);
            };

            onRecordigamiMouseEnter = function _ref4(datum) {
              var wins = datum['win'];
              var losses = datum['loss'];
              var team = datum['team'];
              var year = datum['year'];
              var primaryColor = datum['primary_color'];
              var winPct = getWinPctFromWinsAndLosses(wins, losses);
              hoverSquare.attr("transform", "translate(".concat(parseFloat(datum['x']) + PADDING / 2, ", ").concat(parseFloat(datum['y']) + PADDING / 2, ")")).style("opacity", 1);
              hoverWin.text(wins).style("opacity", 1);
              hoverHyphen.style("opacity", 1);
              hoverLoss.text(losses).style("opacity", 1);
            ...
            onLegendMouseClick = function _ref3(e) {
              ...
              var intervalIndex = intervals.indexOf(intervalStart);
              var intervalClass = decadeClasses[intervalIndex];
              var intervalGroup = intervalGroups[intervalIndex];
              var intervalHighlightColor = decadeHighlightColors[intervalIndex];
              var isExactlyOneTileSelected = isHighlighted.filter(Boolean).length === 1;
              var isHighlightedTileSelected = isHighlighted[intervalIndex];

              if (isExactlyOneTileSelected && isHighlightedTileSelected) {
                d3.select("#hover-square").style("stroke", "white");
                d3.select("#num-recordigami").text("".concat(totalNumRecordigami)).style("opacity", 1);

                for (var i = intervals.length - 1; i >= 0; i--) {
                  var isIntervalHighlighted = isHighlighted[i];

                  if (!isIntervalHighlighted) {
                    highlightGroup(intervalGroups[i], 0, decadeHighlightColors[i]);
                    highlightGroup(legendGroup.selectAll(".decade-".concat(intervals[i])), 0, decadeHighlightColors[i]);
                ...
                var numRecordigami = intervalGroup._groups[0].length;
                ...
                for (var i = intervals.length - 1; i >= 0; i--) {
                  var _isIntervalHighlighted = isHighlighted[i];

                  if (i !== intervalIndex && _isIntervalHighlighted) {
                    fadeGroup(intervalGroups[i], 0, intervalFadeColors[i]);
                    fadeGroup(legendGroup.selectAll(".decade-".concat(intervals[i])), 0, legendFadeColors[i]);
                    isHighlighted[i] = false;
                  }
                }

                d3.select("#hover-square").style("stroke", "black");
                d3.select("#num-recordigami").text("".concat(numRecordigami)).style("opacity", 1);
                highlightGroup(intervalGroup, 0, intervalHighlightColor);
                highlightGroup(legendGroup.selectAll(".".concat(intervalClass)), 0, intervalHighlightColor);
                isHighlighted[intervalIndex] = true;
              }
            };

            bounds.selectAll(".recordigami-tile").remove(); // bounds.selectAll(".voronoi").remove()
            ...
            if (filterTeam === "") {
              ...
              d3.select("#basketball-team-input").style("color", "black");
              d3.select(".typeahead").style("border", "1px solid black").style("border-radius", '30px');
            }

            recordigamiYears = Object.keys(recordigamiData);
            basketballColors = makeColors("#B54213");
            basketballColorScale = d3.scaleThreshold().domain(yearIntervals).range(basketballColors);
            basketballColorContinuousScale = d3.scaleSequential(d3.interpolateSpectral).domain([d3.max(yearIntervals), START_YEAR - 1]);
            grayColors = makeColors(BG_LIGHT_GRAY);
            grayContinuousScale = d3.scaleLinear().domain(yearIntervals).range(grayColors).interpolate(d3.interpolateRgb); // 6. Draw Peripherals

            intervals = getIntervalArray(START_YEAR, END_YEAR, INTERVAL);
            decadeClasses = intervals.map(function (interval) {
              return "decade-".concat(interval);
            });
            decadeHighlightColors = intervals.map(function (interval) {
              return basketballColorContinuousScale(interval);
            });
            decadeHighlightColors[4] = makeColors(decadeHighlightColors[4], 4, 4, 0.2, 0.8)[6];
            intervalFadeColors = decadeHighlightColors.map(function (color) {
              return makeColors(color)[0];
            });
            legendFadeColors = decadeHighlightColors.map(function (color) {
              return makeColors(color)[1];
            });
            decadeFadeColors = decadeHighlightColors.map(function (color) {
              return BG_VERY_LIGHT_GRAY;
            });
            totalNumRecordigami = 0;
            teamNumRecordigami = 0;
            intervalGroups = [];

            _loop = function _loop() {
              var intervalGroup = bounds.append("g");
              var interval = intervals[i];
              var intervalKeys = range(Math.floor(interval / INTERVAL) * INTERVAL, Math.floor(interval / INTERVAL) * INTERVAL + (INTERVAL - 1));
              var intervalData = [];

              for (k = 0; k < intervalKeys.length; k++) {
                if (intervalKeys[k] in recordigamiData) {
                  var intervalNestedKeys = Object.keys(recordigamiData[intervalKeys[k]]);

                  for (j = 0; j < intervalNestedKeys.length; j++) {
                    var dataItem = recordigamiData[intervalKeys[k]][intervalNestedKeys[j]];

                    if (dataItem.team === filterTeam) {
                      teamNumRecordigami += 1;
                      if (intervalKeys[k] in teamRecordigamiData) teamRecordigamiData[intervalKeys[k]][intervalNestedKeys[j]] = dataItem;else {
              ...
              intervalData.flat(1);
              var intervalTiles = intervalGroup.selectAll(".decade-".concat(interval)).data(intervalData).enter().append("rect").attr("class", function (d) {
                return "decade-".concat(interval, " recordigami-tile interval-tile-").concat(winAccessor(d), "-").concat(lossAccessor(d));
              }).attr("height", dimensions.boundedWidth / NUM_GAMES - PADDING).attr("width", dimensions.boundedWidth / NUM_GAMES - PADDING).attr("x", function (d) {
                return xScale(lossAccessor(d)) + PADDING / 2;
              }).attr("y", function (d) {
                return yScale(winAccessor(d)) + PADDING / 2;
              ...
              }).style("stroke-opacity", filterTeam === "" ? 0 : 1).style("fill", function (d) {
                if (filterTeam === DEFAULT_FILTER_NAME) {
                  return decadeHighlightColors[i];
                } else if (filterTeam === "") {
                  return BG_VERY_LIGHT_GRAY;
            ...
            if (![DEFAULT_FILTER_NAME, ''].includes(filterTeam)) {
              onLogoMouseClick = function onLogoMouseClick(clickedTeam) {
              ...
              logoY = -15;
              logoFontSize = 10;
              logoX = -dimensions.margin.left + logoShift / 2 - 10;
              ...
              logoFade = 0.4;
              logo = bounds.selectAll(".team-logo").data(orderedTeamHistory).enter().append("svg:image").attr("class", "team-logo logo").attr("xlink:href", function (team) {
                return "/assets/images/logos/".concat(league, "/").concat(team, ".png");
              }).attr("width", logoSize).attr("height", logoSize).attr("x", logoX).attr("y", function (d, i) {
              ...
              }).attr("class", "team-logo-label logo").attr("x", logoX2).attr("y", function (d, i) {
            ...
            legendTileWidth = Math.min((dimensions.legendWidth - (legendXPadding * intervals.length - 1)) / yearIntervals.length, (dimensions.legendWidth - (legendXPadding * intervals.length - 1)) / (intervals.length - 1));
            legendXRange = Array.from({
              length: intervals.length
            }, function (_, n) {
              return legendX + n * (legendTileWidth + legendXPadding);
            });
            legendXScale = d3.scaleLinear().domain(d3.extent(intervals)).range(d3.extent(legendXRange));
            legendTiles = legendGroup.selectAll(".legend-tile").data(intervals).enter().append("rect").attr("class", function (d, i) {
              return "decade-".concat(d, " legend-").concat(i);
            }).attr("x", function (d, i) {
              return LEAGUE === WNBA && i === 0 ? legendXScale(d) + (legendTileWidth - legendTileWidth / INTERVAL) : LEAGUE === NBA && i === 0 ? legendXScale(d) + (legendTileWidth - legendTileWidth * .4) : legendXScale(d);
            }).attr("y", legendY).attr("width", function (d, i) {
              return LEAGUE === WNBA && i === 0 ? legendTileWidth / INTERVAL : LEAGUE === NBA && i === 0 ? legendTileWidth * .4 : legendTileWidth;
            }).attr("height", dimensions.legendHeight) // .style("fill", d => basketballColorContinuousScale(d))
            .style("stroke", function (d, i) {
              return DEFAULT_FILTER_NAME === filterTeam ? decadeHighlightColors[i] : BG_VERY_LIGHT_GRAY;
            }).style("fill", function (d, i) {
              return DEFAULT_FILTER_NAME === filterTeam ? decadeHighlightColors[i] : BG_VERY_LIGHT_GRAY;
            }); // WNBA filler tile

            if (league === WNBA) {
              wnbaFillerTile = legendGroup.append("rect").attr("class", "filler-tile").attr("x", legendXScale(1995)).attr("y", legendY).attr("width", legendTileWidth - legendTileWidth / INTERVAL).attr("height", dimensions.legendHeight) // .style("fill", d => basketballColorContinuousScale(d))
              .style("stroke", BG_VERY_LIGHT_GRAY).style("fill", BG_VERY_LIGHT_GRAY);
            } // NBA filler tile


            if (league === NBA) {
              nbaFillerTile = legendGroup.append("rect").attr("class", "filler-tile").attr("x", legendXScale(1940)).attr("y", legendY).attr("width", legendTileWidth - legendTileWidth * .4).attr("height", dimensions.legendHeight) // .style("fill", d => basketballColorContinuousScale(d))
              .style("stroke", BG_VERY_LIGHT_GRAY).style("fill", BG_VERY_LIGHT_GRAY);
            }

            if (_isMobile.default.any() && LEAGUE === NBA) {
              legendLabels = legendGroup.selectAll(".recordigami-legend-value").data(intervals).enter().append("text").attr("class", function (d, i) {
                return "decade-".concat(d, " legend-").concat(i);
              }).attr("x", function (d) {
                return legendXScale(d) + legendTileWidth / 2;
              }).attr("y", legendY + dimensions.legendHeight + 10).style("fill", function (d, i) {
                return DEFAULT_FILTER_NAME === filterTeam ? decadeHighlightColors[i] : BG_GRAY;
              }).style("stroke", function (d, i) {
                return DEFAULT_FILTER_NAME === filterTeam ? decadeHighlightColors[i] : "none";
              }).style("stroke-opacity", function (d, i) {
                return DEFAULT_FILTER_NAME === filterTeam ? 1 : 0;
              }).text(function (d, i) {
                return "".concat(d, "s");
              }).attr("text-anchor", "middle").style("alignment-baseline", "middle").style("font-size", legendFontSize).style("fill", "none");
            } else if (_isMobile.default.any() && LEAGUE === WNBA) {
              legendLabels = legendGroup.selectAll(".recordigami-legend-value").data(intervals).enter().append("text").attr("class", function (d, i) {
                return "decade-".concat(d, " legend-").concat(i);
              }).attr("x", function (d) {
                return legendXScale(d) + legendTileWidth / 2;
                ...
                return LEAGUE === WNBA && i === 0 ? "".concat(1997) : "'".concat(d.toString().substr(2), "-'").concat((d + INTERVAL - 1).toString().substr(2));
              }) // : `${d}s`)
              .attr("text-anchor", "middle").style("alignment-baseline", "middle").style("font-size", _isMobile.default.any() && LEAGUE === WNBA ? legendFontSize : legendFontSize).style("fill", "none");
            } else {
              legendLabels = legendGroup.selectAll(".recordigami-legend-value").data(intervals).enter().append("text").attr("class", function (d, i) {
                return "decade-".concat(d, " legend-").concat(i);
              }).attr("x", function (d) {
                ...
                return LEAGUE === WNBA && i === 0 ? "".concat(1997) : LEAGUE === NBA && i === 0 ? '1946-49' : "".concat(d, "-").concat((d + INTERVAL - 1).toString().substr(2));
              }) // : `${d}s`)
              .attr("text-anchor", "middle").style("alignment-baseline", "middle").style("font-size", _isMobile.default.any() && LEAGUE === WNBA ? legendFontSize : legendFontSize).style("fill", "none");
            ...
            isHighlighted = intervals.map(function (interval) {
              return true;
            }); // SEASON_HISTORY[]
            ...
            hoverSquare = bounds.append("rect").attr("class", "rect").attr("id", "hover-square").attr("height", dimensions.boundedWidth / NUM_GAMES - PADDING).attr("width", dimensions.boundedWidth / NUM_GAMES - PADDING).attr("fill", "transparent").attr("x", 0).attr("y", 0).style("opacity", 0).style("stroke", "white").style("stroke-width", "1.5px");
            hoverStartingPointX = dimensions.boundedWidth / 2 - dimensions.margin.left / 2 + dimensions.margin.right / 2;
            hoverWin = bounds.append("text").text('0').attr("class", "record-label").attr("id", 'win-label').attr("x", hoverStartingPointX - 14.5).attr("y", hoverY).attr("text-anchor", "end").style("font-size", hoverFontSize).style("fill", "black").style("opacity", 0);
            hoverHyphen = bounds.append("text").text('-').attr("class", "record-label").attr("id", 'hyphen').attr("x", hoverStartingPointX).attr("y", hoverY).attr("text-anchor", "middle").style("font-size", hoverFontSize).style("fill", "black").style("opacity", 0);
            hoverLoss = bounds.append("text").text('0').attr("class", "record-label").attr("id", 'loss-label').attr("x", hoverStartingPointX + 14.5).attr("y", hoverY).attr("text-anchor", "start").style("font-size", hoverFontSize).style("fill", 'black').style("opacity", 0);
            hoverTeam = bounds.append("text").text('').attr("class", "record-label").attr("x", hoverStartingPointX).attr("y", hoverStartY).attr("text-anchor", "middle").style("font-size", hoverFontSize).style("fill", "black").style("opacity", 0);
            numRecordigamiDescription = bounds.append("text").text('Total # Recordigami').attr("id", "num-recordigami-description").attr("class", "record-label").attr("x", hoverStartingPointX).attr("y", hoverStartY).attr("text-anchor", "middle").style("font-size", hoverFontSize).style("fill", "black").style("opacity", 0);
            numRecordigamiDescriptionBBox = d3.select("#num-recordigami-description").node().getBBox();
            numRecordigamiDescriptionBBoxWidth = numRecordigamiDescriptionBBox.width;
            numRecordigamiLabel = bounds.append("text").text(totalNumRecordigami).attr("id", "num-recordigami").attr("class", "record-label").attr("x", hoverStartingPointX) //numRecordigamiDescriptionBBox.x + numRecordigamiDescriptionBBox.width / 2)
            .attr("y", hoverY).attr("text-anchor", "middle").style("font-size", hoverFontSize).style("fill", "black").style("opacity", 0);
            ...
            if (![DEFAULT_FILTER_NAME, ''].includes(filterTeam)) {
              ...
              if (teamNumRecordigami === 1) {
                bounds.selectAll(".voronoi").remove();
                gameYear = Object.keys(teamRecordigamiData)[0];
                game = Object.keys(teamRecordigamiData[gameYear])[0];
                gameData = teamRecordigamiData[gameYear][game];
                gameMetadata = [{
...
function _updateChart() {
  ...
  /*#__PURE__*/
  regeneratorRuntime.mark(function _callee4(index, classIds, highlightColors, fadeColors, stage, league) {
    var animationTime, i;
          ...
          case 5:
            if (index === -1) {
              ...
              if (stage === "exit") {
                ...
                d3.selectAll(".".concat(classIds[index])).filter(function (d, i) {
                  var isOneAndZero = winAccessor(d) === 1 && lossAccessor(d) === 0;
                  var isZeroAndOne = winAccessor(d) === 0 && lossAccessor(d) === 1;
                  var isLegendLabel = LEAGUE === NBA && d === 1940 || LEAGUE === WNBA && d === 1995;
                  var shouldFade = !isOneAndZero && !isZeroAndOne && !isLegendLabel;
                  return shouldFade;
                }).transition("fade-class-".concat(classIds[index])).duration(animationTime).style("fill", fadeColors[index]).style("stroke", fadeColors[index]).style("stroke-opacity", 0).end();
              }
...
function recircHTML() {
  var storyData = [{
    "image": "wnba_recordigami",
    "url": "html/wnba-recordigami",
