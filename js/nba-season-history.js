...
var winPctAccessor = function winPctAccessor(d) {
  return d.win_pct;
};
      ...
      while (1) {
        switch (_context2.prev = _context2.next) {
          case 0:
            ...
            _context2.next = 3;
            return d3.json("./../assets/data/".concat(league, "_season_win_loss.json"));

          case 3:
            seasonWinLossData = _context2.sent;
            _context2.next = 6;
        ...
        switch (_context3.prev = _context3.next) {
          ...
          case 16:
            ...
            numGames = years.map(function (year) {
              return {
                "num_games": getNumGames(year, league),
                "year": year
              };
            });
            XSCALE = d3.scaleLinear().domain([START_YEAR + 1, END_YEAR]).range([0 + 20, dimensions.boundedWidth - 20]);
...
function updateTopLabel(_x12, _x13, _x14) {
  return _updateTopLabel.apply(this, arguments);
}

function _updateTopLabel() {
    ...
    return regeneratorRuntime.wrap(function _callee4$(_context4) {
      while (1) {
        switch (_context4.prev = _context4.next) {
          case 0:
            firstEraPath = eraPaths[0];
            lastEraPath = eraPaths[eraPaths.length - 1];
            ...
            if (firstEraPath === undefined) {
              ...
              label = player !== '' ? player : team;
              startYear = firstEraPath['start'].toString().substring(2, 4);
              endYear = lastEraPath['end'].toString().substring(2, 4);
              primaryColor = teamData[team]['primary_color'];
              secondaryColor = teamData[team]['secondary_color'];
...
function drawHistoricalArea(team, startYear, endYear, teamWinLossData, teamData, bounds, dimensions, isSelectedTeam, animationTime) {
  ...
  if (!areaExists) {
    ...
    var teamGradientFill = bounds.append("linearGradient").attr("id", teamGradientId).attr("gradientUnits", "userSpaceOnUse").attr("x1", 0).attr("y1", YSCALE(0)).attr("x2", 0).attr("y2", YSCALE(0.8 * YDOMAIN[1])).selectAll("stop").data([{
      ...
      offset: "50%",
      color: "".concat(primaryColor, "88")
    }, {
      offset: "75%",
      color: "".concat(primaryColor, "AA")
    }, {
      offset: "100%",
      color: "".concat(primaryColor, "FF")
    }]).enter().append("stop").attr("offset", function (d) {
      return d.offset;
    }).attr("stop-color", function (d) {
      return d.color;
...
function drawChampionships(id, pathId, filterTeam, teamWinLossData, teamData, bounds, seasonLineGenerator, isSelectedTeam, animationTime) {
  ...
  var starsExist = d3.selectAll("*[id=".concat(id, "-star-]")).nodes().length > 0;
  var championshipsExist = teamChampionshipWinLossData.length > 0;
  var primaryColor = teamData[filterTeam]['primary_color'];
  var secondaryColor = teamData[filterTeam]['secondary_color'];
...
function makeColors(primaryColor) {
  ...
  var pctLighter = arguments.length > 4 && arguments[4] !== undefined ? arguments[4] : 0.64;
...
var fallbackData = [{
  "image": "nba",
  "url": "2020/05/nba-recordigami",
  "hed": "NBA Recordigami"
}, {
  "image": "nba",
  "url": "2020/05/nba-random-walk",
  "hed": "NBA Random Walk"
}, {
  "image": "wnba",
